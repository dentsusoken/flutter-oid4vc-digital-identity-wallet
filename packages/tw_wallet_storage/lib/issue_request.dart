...
class IssueRequest {
  IssueRequest._({required this.id, required this.docType, required this.privateKey});
  ...
  static FutureOr<IssueRequest> create({
    ...
    final pk = await PrivateKey.create(type: privateKeyType, value: keyData);

    return IssueRequest._(id: id ?? const Uuid().v4(), docType: docType, privateKey: pk);
  }

  static FutureOr<IssueRequest?> load({required DataStorageService storage, required String id}) async {
    ...
    return IssueRequest._(id: id, docType: doc.docType, privateKey: pk);
  }
  ...
  Future<CoseKeyPrivate> toCoseKeyPrivate() async {
    final privateKeyX963Data = await TwCrypto.getPrivateKeyX963(privateKey.type, privateKey.value);
    return CoseKeyPrivate.fromData(privateKeyX963Data: privateKeyX963Data, crv: ECCurveType.p256);
  }

  /// get public key
  Future<String> getPublicKeyPEM() {
    return TwCrypto.getPublicKeyPEM(privateKey.type, privateKey.value);
  }
}
