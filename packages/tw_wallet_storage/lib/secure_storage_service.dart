...
class SecureStorageService implements DataStorageService {
  SecureStorageService({
    ...
    _docStorage = FlutterSecureStorage(
      ...
      webOptions: WebOptions(
        dbName: '${serviceName}_doc',
        publicKey: accessGroup ?? 'TWSecureStorage',
      ),
    );
    _logStorage = FlutterSecureStorage(
      iOptions: IOSOptions(
        groupId: accessGroup,
        accountName: '${serviceName}_log',
      ),
      aOptions: AndroidOptions(
        encryptedSharedPreferences: true,
  ...
  FutureOr<Document?> loadDocument(String id) async {
    final doc =
        await _loadDocumentData(id, type: SavedSecureStorageDataType.doc);
    if (doc == null || doc.isEmpty) {
      return null;
    }

    final key =
        await _loadDocumentData(id, type: SavedSecureStorageDataType.key);
    if (key == null || key.isEmpty) {
      return null;
    }

    return _makeDocument(
      jsonKey: key,
      jsonDoc: doc,
    );
  }
  ...
  FutureOr<List<PresentationLog>?> loadPresentationLogs() async {
    ...
    final logs = items.values
        .map((e) => PresentationLog.fromJson(jsonDecode(e)))
        .toList();
  ...
  FutureOr<void> saveDocument(
    ...
    return _saveDocumentData(
      document,
      dataToSaveType: SavedSecureStorageDataType.doc,
  ...
  FutureOr<void> deleteDocument(String id) async {
    await _deleteDocumentData(id,
        dataToSaveType: SavedSecureStorageDataType.doc);
    await _deleteDocumentData(id,
        dataToSaveType: SavedSecureStorageDataType.key);
  }
...
extension KeyChainStorageServiceExt on SecureStorageService {
  Document _makeDocument({
    ...
    required String jsonDoc,
  }) {
    final key = PrivateKey.fromJson(jsonDecode(jsonKey));
    final doc = Document.fromJson(jsonDecode(jsonDoc));
    return doc.copyWith(privateKey: key);
  }

  FutureOr<String?> _loadDocumentData(
    String id, {
    required SavedSecureStorageDataType type,
  }) {
    final storage = _storageForType(type);
    return storage.read(key: id);
  }

  FutureOr<Map<String, String>> _loadDocumentsData(
      SavedSecureStorageDataType type) {
    final storage = _storageForType(type);
  ...
  FutureOr<void> _saveDocumentData(
    ...
    if (dataToSaveType == SavedSecureStorageDataType.key &&
        !document.hasPrivateKey) {
  ...
  FutureOr<void> _deleteDocumentData(
    String id, {
    required SavedSecureStorageDataType dataToSaveType,
  }) {
    final storage = _storageForType(dataToSaveType);
